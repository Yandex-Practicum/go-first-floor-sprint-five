...
func (t Training) meanSpeed() float64 {
	...
	return distances / float64(t.Duration.Hours())
}
...
func (t Training) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Distance:     t.distance(),
		Speed:        t.meanSpeed(),
		// Calories field will be updated when calling the Calories() method in ReadData
	}
}
...
func (r Running) Calories() float64 {
	...
	result := (CaloriesMeanSpeedMultiplier*avgSpeed + CaloriesMeanSpeedShift) * (r.Weight / float64(MInKm)) * float64(r.Duration.Hours()) * float64(MinInHours)
	return result
}
...
func (r Running) TrainingInfo() InfoMessage {
	// Вызов метода TrainingInfo для структуры Training, встроенной в Running
	// Возврат структуры InfoMessage с информацией о тренировке бега
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	result := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * float64(MinInHours))
	return result
}
...
func (s Swimming) Calories() float64 {
	...
	result := float64(s.LengthPool) * float64(s.CountPool) / MInKm / float64(s.Duration.Hours())
	return result
}

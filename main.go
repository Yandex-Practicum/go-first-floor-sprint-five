...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	calories := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / float64(MInKm)) * r.Duration.Hours() * float64(MinInHours)
	return calories
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	speedMps := w.meanSpeed() / 3.6 // переводим среднюю скорость из км/ч в м/с
	calories := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMps, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * float64(MinInHours))
	return calories
}

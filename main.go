...
const (
	MiInKm     = 1000 // количество метров в одном километре
...
func (t Training) distance() float64 {
	...
	return float64(t.Action) * t.LenStep / MiInKm
}
...
const (
	CaloriesMeanSpeedMultiplier = 18.00

	// множитель средней скорости бега
...
func (r Running) Calories() float64 {
	speed := r.meanSpeed()
	return (CaloriesMeanSpeedMultiplier*speed + CaloriesMeanSpeedShift) * r.Weight / MiInKm * r.Duration.Minutes() / MinInHours
}
...
func (w Walking) Calories() float64 {
	speed := w.meanSpeed()
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(speed/KmHInMsec, 2)/w.Height*CaloriesSpeedHeightMultiplier)*w.Weight) * w.Duration.Minutes() / MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
	totalDistance := float64(s.LengthPool*s.CountPool) / 1000 // переводим в км
	totalTimeInHours := s.Duration.Hours()
...
func ReadData(training Training) string {
...
func main() {
	...
	fmt.Println(ReadData(swimming.Training))
	...
	fmt.Println(ReadData(walking.Training))
	...
	fmt.Println(ReadData(running.Training))

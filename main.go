...
func (t Training) distance() float64 {
	return float64(t.Action) * float64(t.LenStep) / MInKm // Дистанция в км
}

// meanSpeed возвращает среднюю скорость бега или ходьбы.
func (t Training) meanSpeed() float64 {
	if t.Duration.Hours() == 0 { // Проверка на нулевое время
		return 0
	}
...
type Walking struct {
	Training
	Height float64 // Рост спортсмена
}
...
func (s Swimming) meanSpeed() float64 {
	if s.Duration.Hours() == 0 { // Проверка на нулевое время
		return 0
	}
...
func (s Swimming) TrainingInfo() InfoMessage {
	return InfoMessage{
		...
		Duration:     s.Duration,
		Distance:     s.distance(),
		Speed:        s.meanSpeed(), // Переопределенный метод
		Calories:     s.Calories(),  // Переопределенный метод
	}
}

...
func (r Running) Calories() float64 {
	return ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours) // время_тренировки_в_часах (уточнить является ли duration этой переменной)
}
...
func (w Walking) Calories() float64 {
	...
	speedHeight := w.meanSpeed() * KmHInMsec

	return (CaloriesWeightMultiplier*w.Weight + ((math.Pow(speedHeight, 2) / heughtInMetrs) * CaloriesSpeedHeightMultiplier * w.Weight)) * w.Duration.Hours() * MinInHours
}

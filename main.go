...
func (t Training) distance() float64 {
	...
	NewDistance := float64(t.Action) * t.LenStep / MInKm
	return NewDistance
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	SpentRunningCalories := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours()) * MinInHours
	return SpentRunningCalories
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	SpentWalkingCalories := (CaloriesWeightMultiplier*w.Weight + math.Pow(w.meanSpeed()*KmHInMsec, 2)/(w.Height/CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
	return SpentWalkingCalories
}
...
func (s Swimming) meanSpeed() float64 {
	...
	MidSwim := float64(s.LengthPool) * float64(s.CountPool) / MInKm / s.Duration.Hours()
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	SpentSwimCalories := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()

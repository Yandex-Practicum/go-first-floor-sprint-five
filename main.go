...
func (w Walking) Calories() float64 {
	...
	calories := (CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMetersPerSecond, 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
	return calories
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	info := s.Training.TrainingInfo() // Получаем базовую информацию из Training

	// Теперь добавим информацию, специфичную для Swimming
	swimmingSpeed := s.meanSpeed()
	info.Speed = swimmingSpeed
	// почему именно так должно быть, не очень понимаю, опишите пожалуйста подробнее
	return info
}
